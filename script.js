...
function reservarArmario() {
  ...
  // Salvar a data da reserva como um objeto Date
  armarioSorteado.dataReserva = new Date();

  // Adicionar 24 horas (tempo de entrega)
  let dataEntrega = new Date(armarioSorteado.dataReserva.getTime() + 24 * 60 * 60 * 1000);

  // Formatar para exibição (DD/MM/YYYY HH:MM:SS)
  let dataReservaFormatada = `${armarioSorteado.dataReserva.toLocaleDateString()} ${armarioSorteado.dataReserva.toLocaleTimeString()}`;
  let dataEntregaFormatada = `${dataEntrega.toLocaleDateString()} ${dataEntrega.toLocaleTimeString()}`;

  armarioSorteado.dataReservaFormatada = dataReservaFormatada;
  armarioSorteado.dataEntregaFormatada = dataEntregaFormatada;
  ...
  document.getElementById("resultado").innerText = `Olá, ${usuario.nome}! O armário ${armarioSorteado.id} foi reservado com sucesso! A data limite para entrega e: ${armarioSorteado.dataEntregaFormatada}`;

  console.log(usuario);
  console.log(armarios);
  console.log(armarioSorteado.dataReservaFormatada)
